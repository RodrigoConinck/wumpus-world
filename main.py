...
def MoveToUnvisited(ag, visited, goalLoc, dfsVisited):
...
def ExitWumpusWorld(ag, kb):
    visited = [False for i in range(16)]  # Posições visitadas até o momento
    while (ag.FindCurrentLocation() != [4, 4]):
        ...
        if percept[0] == True:  # briza
            breezeClause[curLocIndex + 48] = 1
        else:
            breezeClause[curLocIndex + 48] = -1
        kb.AddClause(breezeClause)  # presença ou não da briza/noia

        if percept[1] == True:  # fedor
            stenchClause[curLocIndex + 16] = 1
        else:
            stenchClause[curLocIndex + 16] = -1
        kb.AddClause(stenchClause)  # presença ou não do fedor
